	...
	<view class="page">
		...
		<view class="card">
			...
			<view class="input-box">
				...
				<input v-model="phone" type="number" placeholder="请输入手机号码" class="input" @input="handleInput" />
			</view>
			...
			<view class="input-box">
				...
				<input v-model="password" password placeholder="请输入6-18位密码" class="input" @input="handleInput" />
			</view>
			...
			<view class="input-box">
				<image src="/static/login_passworld.png" class="icon" mode="aspectFit" />
				<input v-model="confirmPassword" password placeholder="请再次输入6-18位密码" class="input" @input="handleInput" />
			</view>

			<!-- 协议 -->
			<view class="agreement">
				<view class="checkbox" @click="agree = !agree">
					<image :src="agree ? '/static/selectionbox_choose.png' : '/static/selectionbox_nochoose.png'" class="check-icon" />
				</view>

				<text class="gray">注册即表示同意</text>

				<text class="link" @click="goProtocol('用户服务协议')">《用户服务协议》</text>

				<text class="gray">和</text>

				<text class="link" @click="goProtocol('隐私政策')">《隐私政策》</text>
			</view>
